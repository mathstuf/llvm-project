...
export module foo;
  ...
  EXPECT_TRUE(PP->isNamedModule());
  EXPECT_TRUE(PP->isNamedInterfaceUnit());
  EXPECT_FALSE(PP->isImplementationUnit());
...
module foo;
  ...
  EXPECT_EQ(Callback->importNamedModuleNum(), 0);
  EXPECT_TRUE(PP->isNamedModule());
  EXPECT_FALSE(PP->isNamedInterfaceUnit());
  EXPECT_TRUE(PP->isImplementationUnit());
  EXPECT_EQ(PP->getNamedModuleName(), "foo");
}
